...
默认的 Linux 构建使用 GLFW + OpenGL，并启用 X11、Wayland、GIO 托盘和音频支持。全新 Ubuntu / Debian 系统可以直接复制下面的命令安装所需开发包：
...
GLFW、glad、FreeType、zlib、libpng、md4c 和 miniaudio 已随 EUI-NEO 源码提供，不需要另外安装对应的开发包。`libasound2` 和 `libpulse0` 属于运行时音频库；如果目标机没有桌面音频环境，再安装：

```sh
sudo apt-get install -y libasound2 libpulse0
```

如果不需要系统托盘，可以在配置时关闭托盘支持，这样可以省略 `libglib2.0-dev`：
