...
inline void queuePointerInput(window::Handle window, double x, double y, bool down, bool rightDown = false) {
...
inline bool hasPendingPointerInput(window::Handle window, float dpiScale = 1.0f) {
    ...
    return x != state.lastX ||
           y != state.lastY ||
           down != state.lastDown ||
           (state.hasQueuedPointer ? state.queuedRightDown : core::window::isMouseButtonDown(window, 1)) != state.lastRightDown;
}
...
inline PointerEvent readPointerEvent(window::Handle window, float dpiScale = 1.0f) {
    ...
    if (state.hasQueuedPointer && !state.queuedDown && !state.queuedRightDown) {
